...
- **Workspace level** is anything that would still hold in a different project: a line in the workspace agent rules, a skill change, an environment improvement. Append an entry to the ledger in the form its own header defines (the header is the only place that format lives); the type is one of the seven environment categories above, or `rule` for a candidate line in the agent rules, or `skill` for a skill change, or a type the header adds. Write the event ("finding the report script took 20 minutes"), not the fix; the fix is inferred when two events line up. The ledger is the nearest `LEDGER.md` found walking up from the working directory; if none, create it beside the outermost `AGENTS.md`. Propose a promotion only when the ledger already holds the same event from another session. Delete an existing instruction only with evidence it caused a wrong action.
